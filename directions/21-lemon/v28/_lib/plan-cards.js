...
(function () {
  ...
  function renderCard(plan, isSelected) {
    ...
    // V27 F05 — was h4. At COMPARE phase the h2 known-list is hidden, so h1→h3
    // still skipped. Promoted to h2 (sibling to known-list) so the visible-heading
    // hierarchy is always h1 (dash-headline) → h2 (plan-card or known-list) → h3
    // (compare modal / plan report) regardless of which phase is active.
    var headlineBlock =
      '<h2 class="plan-card__short">' + escapeHtml(shortTierName) + '</h2>';
